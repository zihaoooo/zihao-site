...
- Image optimization: max 1600px wide, JPEG q85 — ship JPEG (flatten alpha onto white). Keep PNG only where transparency is load-bearing, e.g. Seed Bomb's `flowercolalge_bottom.png`, which overlaps the content above it; use ffmpeg (Python unavailable)
